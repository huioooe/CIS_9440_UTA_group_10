-- Clean and standardize 311 service request noise complaint data
-- One row per service request

WITH source AS (
   SELECT * FROM {{ source('raw', 'source_311_subset_noise') }}
...
cleaned AS (
   SELECT
       ...
       * EXCEPT (
       ...
       CAST(complaint_type AS STRING) AS problem_type,
...
SELECT * FROM cleaned
-- All should be part of this table: stg_nyc_311_noise